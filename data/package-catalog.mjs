...
export const packageCatalog = [
  {
    ...
    adoptionStage: 1,
    homeSummary: {
      en: 'PostgreSQL RLS + Prisma multi-tenancy with live database audits and non-HTTP context safeguards.',
      ko: 'PostgreSQL RLS + Prisma 멀티테넌시와 실시간 DB 감사, 비HTTP 컨텍스트 보호를 제공합니다.',
    },
    solves: 'Tenant context, PostgreSQL RLS, Prisma 6/7 query isolation, live database audits, and tenant-scoped non-HTTP resources.',
  ...
  },
  {
    ...
    adoptionStage: 3,
    homeSummary: {
      en: 'Tenant-scoped API keys with Prisma 5/6 storage, zero-downtime rotation, IP allowlists, lifecycle hooks, and verification metrics.',
      ko: 'Prisma 5/6 저장소, 무중단 rotation, IP allowlist, lifecycle hook, 검증 metric을 갖춘 테넌트 범위 API 키입니다.',
    },
    solves: 'Tenant-scoped API keys, verified Prisma 5/6 storage, rotation, IP allowlists, lifecycle hooks, and verification metrics.',
  ...
  },
  {
    ...
    adoptionStage: 4,
    homeSummary: {
      en: 'Typed jobs with bootstrap-safe handler discovery, tenant-fair in-memory scheduling, restart-safe BullMQ workers, Redis dedupe, retries, and outbox publishing.',
      ko: '부트스트랩 안전 handler 검색, 테넌트 공정 in-memory scheduling, 재시작 안전 BullMQ worker, Redis dedupe, retry, outbox 발행을 갖춘 typed job 실행입니다.',
    },
    solves: 'Typed job contracts, bootstrap-safe handler discovery, tenant-fair local scheduling, restart-safe BullMQ execution, Redis idempotency/dedupe, retries, graceful shutdown, and outbox publishing.',
